...
- **F2.3** Chord per beat: HPCP chroma frames (band preset off, so the bass register is weighed by its energy rather than normalised on its own and counted as much as everything above it) are averaged per beat and decoded into major/minor triads with a Viterbi pass. The emission at each beat is a softmax posterior over the 24 templates (temperature 4, root and third weighed equally, fifth 0.95) rather than a raw similarity, so a beat's evidence weighs the same against the change penalties whether it was loud or muddy. Chord changes are cheap on estimated downbeats and expensive mid-bar, and diatonic chords get a small bonus. Beats whose signal is under 3 % of the song's median level (a silent intro, the fade-out) are "N", no chord. Measured on eight synthesised band recordings (strummed chords with sevenths, a bass line on fifths and walk-ups, a melody with passing tones, drums): 92.6 % of the time the right chord, against 69.9 % for the previous decoder; through the app on one of them, 99.9 %.
...
- **F4.3** Transpose ±11 semitones and capo 0–7 (guitar/ukulele). Display updates instantly; enharmonic spelling follows the key. The recording follows the transpose setting too ("Transpose the sound too", on by default, in the song settings sheet): the player's audio is routed through a phase-vocoder pitch shifter in an AudioWorklet (`public/workers/pitch-processor.js`; 4096-point frames, 75 % overlap, identity phase locking on spectral peaks, phase reset on transients) that shifts the pitch by the transpose amount at the same speed. The capo changes only the fingering, never the sound. The graph is built the first time a shift is asked for and stays; at zero it passes the sound straight through. The shifter's latency (about 90–130 ms depending on the ratio) is subtracted from the playhead the sheet and metronome follow. Measured: notes within 1.2 cents at every shift from −11 to +11, chords recognised as the transposed chord, level within 1 dB, 60× real time in Node.
...
- **F4.6** Simplify chords (song settings sheet, opened from the first control in the song's settings strip): a six-stop slider — as detected, every 2 beats, every bar, every 6 beats, every 2 bars, one per lyric line. Each stretch keeps one chord, by default the one that sounds longest in it (weighted by strength), or the one sounding when the stretch starts ("Which chord stays: Longest / First"). Stretches line up with the detected bar lines; "one per line" uses the lyric lines (synced, or as the sheet places unsynced ones), with two-bar stretches before the first line and inside any line longer than three bars, and falls back to every 2 bars without lyrics. A separate switch, "Only chords from the key", folds any chord outside the key's six triads (plus the major V in minor) into the chord before it. The sheet shows the resulting number of chord changes as the slider moves. The simplification is a view: the analysis keeps every chord, and the sheet, timeline, chord gallery, Learn view and the player's now/next all follow the chosen level. Settings are global preferences.
- **F4.7** Hide controls while reading (on by default, song settings sheet): on phones, scrolling the sheet down slides the song header (title, view switch, Tutorial button) up off the screen and the player down off it; scrolling up, or tapping anywhere, brings them back, and at the top of the page they are always shown. Only scrolling the person does counts (wheel, touch, keyboard within the last half second): the sheet following the song, or a tapped line scrolling into view, never hides anything. The tab bar on the other screens does the same. Hidden controls are `inert`. Nothing hides on desktop, where everything is docked.
...
## 16. Build notes (2026-09-19)
- Chord accuracy (F2.3). Built an offline benchmark first: eight synthesised "band" songs with known chords (a bass on fifths and walk-ups, melody passing tones, sevenths, drums) run through the real worker in Node. The previous decoder scored 69.9 %, its errors mostly relative major/minor swaps and missed half-bar changes. One HPCP parameter did most of the work: the band preset normalises everything under 500 Hz on its own and adds it with the weight of the whole rest, so a bass note on the fifth or a walk-up dragged the chord with it. Off, 84.8 %; with an equal-weight third and a softmax posterior as the emission, 92.6 %. Harmonic templates, seventh templates, median chroma and heavier penalties all measured worse and were not kept. Silent beats are now "N". Through the app on one of the songs: 99.9 %, key and tempo right, the tail silent.
- Transpose now changes the sound (F4.3): a phase vocoder in an AudioWorklet. 2048-point frames left three bass-register notes a tone apart sharing one peak and mistuned one of them; 4096-point frames keep each its own. Node tests: every note within 1.2 cents, four chords recognised as their transposed selves, level within 1 dB, a 27 ms gap once when the shift changes mid-play. In the browser (offline render through the real module): within 0.7 cents. The element is routed through the graph only when a shift is first asked for, and rebuilt if the AudioContext is replaced (iOS after an interruption), since an element can be tied to one context for life.
- Simplify chords (F4.6) and hide-while-reading (F4.7) as above. Twenty Node checks on the simplifier (bar alignment, longest vs first, in-key folding including a foreign opening chord, line mode with intro and long-line splitting, silence), and browser runs: slider stops, change counts, saved settings, every chord on the page in key when asked; hiding on a finger scroll, returning on scroll-up and tap, staying put through programmatic scrolls, always shown at the top, off when the setting is off.
- Headless Chrome on this Mac cannot decode AAC, so browser playback tests use a WAV song; the benchmark WAV (`public/__test/band.wav`) doubles as that song.
