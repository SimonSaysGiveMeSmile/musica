    ...
    <div>
      ...
      <div className={`grid gap-3 ${instrument === "piano" ? "grid-cols-1 sm:grid-cols-2 lg:grid-cols-3" : "grid-cols-2 sm:grid-cols-3 lg:grid-cols-4"}`}>
        {shown.map((c) => {
          ...
          return (
            <button key={c} onClick={() => onChord(c)} className={`press lacquer rounded-[22px] p-3 flex flex-col items-center gap-1 relative overflow-hidden transition-shadow ${hot ? "ring-1 ring-gold-hi/70" : ""}`}>
              ...
              <div className="flex items-center gap-2 self-stretch justify-between">
                <span className={`chordname text-[22px] ${hot ? "text-gold-hi" : "text-ivory"}`}>{c}</span>
                <span className={`text-[10px] font-medium tracking-wider uppercase ${unknown ? "text-ivory-3" : "text-gold"}`}>{unknown ? "learn" : "known"}</span>
              </div>
