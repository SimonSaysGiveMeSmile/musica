 ...
 *  and a microphone that pauses the song the moment you stop playing. */
...
import { useSynthPlayer, previewNote } from "@/lib/audio/synth";
...
import { IconBack, IconLive, IconNotes, IconPause, IconPlay, IconRewind, IconSectionBack, IconSliders } from "@/components/ui/Icons";
...
export function TutorialView({ id }: { id: string }) {
  ...
  const detect = usePlayDetect({ enabled: wantMic, sensitivity: prefs.pauseSensitivity, onIdle, onActive });
  const setAutoPause = useCallback((on: boolean) => { markPaused(false); setPrefs({ autoPause: on }); }, [markPaused]);

  /* ---------------- transport helpers ---------------- */
  const sections = useMemo(() => {
    if (tutorial?.sections?.length) return tutorial.sections;
    return [0];
  }, [tutorial]);

  const rewind = useCallback(() => {
    markPaused(false);
    player.seek(Math.max(0, player.now() - prefs.rewindSec));
  }, [player, prefs.rewindSec, markPaused]);
  ...
  const backSection = useCallback(() => {
    ...
    player.seek(player.now() - here.start > 1.5 ? here.start : here.prev);
  }, [player, sections, markPaused]);

  const manualToggle = useCallback(() => { markPaused(false); player.toggle(); }, [player, markPaused]);
  ...
  const build = useCallback(async (mode: "arrange" | "transcribe") => {
    ...
    try {
      ...
      const built = await generateTutorial(blob, { mode, analysis, onProgress: (pct) => setBusy({ pct }) });
  ...
  const importScore = useCallback(async (file: File) => {
    ...
    try {
      ...
      const built = tutorialFromScore(parsed, analysis);
  ...
  const chordSpans: ChordSpan[] = useMemo(() => {
    if (!analysis || instrument === "piano") return [];
    const out: ChordSpan[] = [];
    ...
    return out;
  }, [analysis, instrument, shift, flats]);

  const getTime = useCallback(() => player.now(), [player]);
  const look = FALL.find((f) => f.look === prefs.tutorialSpeed)?.look ?? 3;
  const pianoReady = instrument === "piano" && !!tutorial && notes.length > 0;
  const fretReady = instrument !== "piano" && chordSpans.length > 0;
  const current = analysis && instrument !== "piano" ? chordAt(analysis, player.time) : null;
    ...
    <main className="h-dvh flex flex-col overflow-hidden" style={{ paddingBottom: "calc(var(--sab) + 6px)" }}>
      ...
      <header className="safe-top px-4 pt-1 pb-2 flex items-center gap-3 shrink-0 lg:px-8">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="ios-caption label-2 truncate">{t("tut.title")}{instrument === "piano" && tutorial ? ` · ${t(`tut.src.${tutorial.source}` as const)}` : ""}</div>
        </div>
        {song.hasAudio && notes.length > 0 && (
      ...
      <section className="flex-1 min-h-0 px-3 lg:px-8 relative">
        <div className="inset-group h-full relative">
          ...
          {fretReady && (
            <FretRoll chords={chordSpans} instrument={instrument === "ukulele" ? "ukulele" : "guitar"} getTime={getTime}
            ...
            <div className="h-full flex flex-col items-center justify-center text-center px-6 gap-4">
              {busy ? (
                ...
                <>
                  ...
                  <p className="ios-footnote label-2 max-w-[300px]">{instrument === "piano" ? t("tut.pianoIntro") : t("tut.noChords")}</p>
                  {instrument === "piano" && (
                    <div className="flex flex-col gap-2 w-full max-w-[320px] mt-1">
                      <Choice title={t("tut.arrange")} hint={t("tut.arrangeHint")} onClick={() => build("arrange")} primary />
                      <Choice title={t("tut.transcribe")} hint={t("tut.transcribeHint")} onClick={() => build("transcribe")} />
      ...
      <div className="px-3 pt-2 lg:px-8 shrink-0">
        <div className="glass-strong rounded-[28px] px-3 py-2.5">
          <Scrubber duration={duration} time={player.time} sections={sections} onSeek={(x) => { markPaused(false); player.seek(x); }} />
          <div className="flex items-center justify-between mt-2">
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => player.setRate(player.rate <= 0.55 ? 1 : Math.round((player.rate - 0.15) * 100) / 100)}
                className="press glass rounded-full h-11 px-3 ios-footnote tabular-nums text-ivory" aria-label={t("player.speed")}>
            ...
            <span className="ios-caption label-3 tabular-nums w-11 text-right">{fmtTime(duration)}</span>
          </div>
          {(note || detect.error) && (
        ...
        {instrument === "piano" && (
          <div className="inset-group mt-4">
            <button onClick={() => build("arrange")} disabled={!!busy} className="row press w-full text-left">
              <span className="ios-body flex-1">{t("tut.arrange")}</span><span className="ios-footnote text-gold">{t("tut.make")}</span>
            </button>
            ...
            {tutorial && (
              <div className="row !min-h-0 py-2">
                <span className="ios-caption label-3">
                  {t("tut.fingers")}{tutorial.dropped > 0 ? ` · ${t("tut.dropped", { n: tutorial.dropped })}` : ""}
                </span>
              </div>
            )}
          </div>
...
function Scrubber({ duration, time, sections, onSeek }: { duration: number; time: number; sections: number[]; onSeek: (t: number) => void }) {
  ...
  return (
    <div ref={ref} onPointerDown={(e) => at(e.clientX)} onPointerMove={(e) => { if (e.buttons) at(e.clientX); }}
      ...
      {duration > 0 && sections.map((s) => (
        <span key={s} aria-hidden className="absolute w-px h-2.5 rounded-full" style={{ left: `${(s / duration) * 100}%`, background: "color-mix(in srgb, var(--ivory) 30%, transparent)" }} />
      ))}
    </div>
  );
}
